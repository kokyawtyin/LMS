...
function search_perform($terms, $table, $field) {
	...
	$sql = "SELECT gr.id, gr.title, cat.name, aut.authorname, gr.cover FROM $table as gr INNER JOIN categories as cat ON gr.category_id = cat.id INNER JOIN author as aut ON gr.authorid = aut.authorid WHERE $parts";
	$rows = array();
